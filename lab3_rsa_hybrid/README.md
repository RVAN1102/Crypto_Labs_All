...
- RSA key generation for RSA-3072 and RSA-4096 in DER or PEM format.
...
Generate PEM keys and key metadata:

```bat
build\rsatool.exe keygen --bits 3072 --priv private.pem --pub public.pem --meta key_metadata.json
```
...
Assignment-compatible automatic encryption:

```bat
build\rsatool.exe encrypt --pub public.der --in plain.bin --out encrypted.bin --label-text "lab3"
build\rsatool.exe decrypt --priv private.der --in encrypted.bin --out recovered.bin --label-text "lab3"
```

`encrypt` writes direct RSA-OAEP ciphertext when the input fits the RSA-OAEP(SHA-256) limit. For larger files, it automatically uses hybrid AES-256-GCM and writes an envelope to `OUT.envelope.json` unless `--envelope env.json` is provided. `decrypt` uses hybrid mode when `--envelope` is provided, when `IN.envelope.json` exists, or when `OUT.envelope.json` exists; otherwise it uses direct RSA-OAEP.
...
All commands that load RSA keys accept DER or PEM. File content is inspected for PEM markers, and `.pem` extensions are decoded as PEM. Existing `.der` files continue to use DER behavior.
...
```bat
build\rsatool.exe bench --out artifacts\windows\bench\bench_windows_raw.csv --summary artifacts\windows\bench\bench_windows_summary.csv --runs 10 --ops 10 --sizes 1k,16k,256k,1m --rsa-sizes 32,190,318 --rsa-bits 3072,4096 --platform windows-mingw64
```
...
```powershell
powershell -ExecutionPolicy Bypass -File scripts\report\run_lab3_windows_evidence.ps1
```

Ubuntu/Linux:

```bash
bash scripts/report/run_lab3_ubuntu_evidence.sh
```

The evidence runners create matching standardized logs under `artifacts/windows/logs/` and `artifacts/linux/logs/`, including CTest, unit tests, KAT, the 40-case negative-test matrix, base benchmark logs, Hybrid 100 MiB benchmark logs, artifact inventory, and verification summary. The cross-platform matrix is documented in `report/lab3_test_matrix.md`.
...
`keygen --meta key_metadata.json` writes:
...
- [x] DER and PEM key files supported.
- [x] Optional key metadata JSON supported.
- [x] Assignment-compatible `encrypt`/`decrypt` auto mode supported.
...
- PEM support wraps the same DER key material generated by Crypto++.
